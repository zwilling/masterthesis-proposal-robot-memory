...
takes care of the dement, old man Frank to allow him a happy and
...
with preconditions and effects \todo{precond+eff drin lassen?} can be
executed to reach a goal.

The basic requirements of a robot memory are representing and storing
knowledge and providing it again when beeing queried. With knowledge,
...
object positions and action recepies (knowledge), and learned
probability distributions (wisdom). This includes spatio-temporal
knowledge such as position coordinates with timestaps as well as
...
strenghts. For example it is convinient to use a PDDL based planner to
find a global plan achieving a goal, a reasoner to execute plans and
update the world model, and a motion planner for grasping and drivng
...
allows sharing knowledge consistently and avoids ...


\begin{enumerate}
\item Domäne -> Anforderungen
\item Anforderungen
  \begin{enumerate}
  \item Derive Symbolic K
  \item ~Planning challenge
  \item Share K (planners/robots)
  \item Persistance
  \item State Estimation
  \end{enumerate}
Before these applications can be realized, important
challenges need to be solved.  Also planning
...
planners and reasoners to share the same world model.
\item Useage of K: allgemein -> speziell + Filter/Update/generierung
\item K about different stuff (Info,Action) -> represented in structure
\item Combination of strengths
  This way
...
is a problem. For example when a planner creates plans and a reasoner
executes them, the reasoner might cancel the plan because of a newly
discovered obstacle. Without a shared worldmodel or at least a notification,
the planner could create the same plan again and would be stuck.

\item Geom Motion Planner (Clolli) Hybrid
\item Connection Temporal/Geometric K -Symbolic K
\item ONE state estimation for all Apps
\item Shared RM generates KB on different robots
  The robot memory is also needed for
multiple robots sharing a common knowledge base for
proper cooperation (e.g. to know which tasks are already done by other
robots or where another robots placed an object). The robot memory frees planners
...
\item VKB (name) Computable knowledge, on-demand (computed) knowledge, Computables, computable documents
...
precondition is no longer satisfied.
\item MongoDB
\item Fawkes
...
For the time of the thesis, we focus on two applications which can
especially profit from the robot memory and will be used for
evaluation. On the one hand, the robot memory should be used on
logistics robots of the RoboCup Logistics League (RCLL). Here it
should enable sharing the world model between a global planner and a
reasoner for plan execution and between the multiple robots
collaborating as in a team. --only prototype poc-- On the other hand, the robot memory should
be used on a domestic service robot related to the RoboCup@Home league
where the robot memory is needed as flexible and scalable long-time
storage of various facts about a domestic environment, how to perform
actions and to learn spatio-temporal distributions from observed
object positions.
...
reasoners which should profit from using the robot memory and
...
perception, reasoning, and behavior execution. The binary
...
Fawkes guarantees loop times by
...
for accessing logging, timing, and external libraries~\cite{tnthesis}.

The communication between plugins is realized with the \emph{blackboard}
...
For examples sensor and actor plugins can be exchanged by simulation
...
obstructive for multiple components sharing common information.
%% For
...
going to profit from the robot memory. For the variety of these, we
...
\texttt{state}, \texttt{visible}, and \texttt{position} facts where
$?$ and $\$?$ mark variables. The consequent behind $=>$
...
The inference engine checks which
rule antecedents are satisfied. If multiple rules
...
The RCLL agent implemented in CLIPS represents its
...
and features. A version used in RoboCup domains is
...
%% storing them.
The concept inspired the usage of virtual knowledge
bases in this thesis.
...
it to the virtual knowledge base, which
...
the query to check if there is virtual knowledge provided for it.

...
we mean the generalization of the
Data-Information-Knowledge-Wisdom (DIKW) hierarchy~\cite{DIKW} ranging
...
memory should be used by logistics robots of the RoboCup Logistics
...
robot related to the RoboCup@Home league. Here the robot memory is
needed as flexible and scalable long-time storage used by different
components depending on either spatio-temporal or symbolic
representations of stored knowledge.
...
transferred into a planner or reasoner. It follows an overview of the
...
reasoners.  This allows combining them with their specialized
...
documents containing themselves are not considered because they are not
...
robots. To perform the qualitative evaluation, we will implement

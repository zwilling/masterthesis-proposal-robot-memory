...
the master election when necessary. \todo{sharding} Using replication,
...
found at which positions\todo{cite debul proposal}). Thus the robot
...
this proposal we focus on the integration into PDDL. The theoretical
foundation can be used to formalize the integration into Golog, but
this exceeds the scope of the proposed thesis.
...
robot memory document oriented, we first define documents and their
...
\\
A \textbf{database} $\mathcal{DB}$ is a set of documents with unique object
identifiers, which are a special key-value pair: \todo{unique ids rauskürzen?}
\begin{align*}
...
abstraction~\cite{software-architecture}\todo{nagl ref?}, to allow

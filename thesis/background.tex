...
\subsection{Knowledge Representation, Reasoning, and Planning}
\label{sec:reasoning}
...
knowledge can be \emph{logically infered} from other knowledge, by
representing the propositions, which describe what is known or
beliefed, and then performing reasoning manipulations on it.
A simple example for logical inference is the following:
\begin{enumerate}
\item Ceasar is a robot
\item All robots should have a memory
\item Therefore, Ceasar should have a memory
\end{enumerate}
The field of knowledge representation deals with the question how
propositions like these can be represented in logic formalisms, such
as First-Order Logic (FOL). \todo[inline]{introduce FOL?}  Knowledge
representation and reasoning are important parts of artificial
intelligence in mobile robotics. Knowledge representation is used to
model and memorize what a robot knows or beliefs about its
environment and reasoning is used to infer what effects certain
actions can have on the environment and which informations can be
infered by made observations.



rules: premises and conclusions
...
robot memory source of knowledge for reasoning and storage for results  

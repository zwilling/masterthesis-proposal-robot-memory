...
\emph{sense-think-act cycle}. Sensors are used to perceive the
...
the robots behavior. The architecture of a multi-agent system can be
...
manipulation of symbols representing a believed propositions to
...
such as First-Order Logic (FOL). \todo[inline]{introduce FOL?}
...
reasoning and incremental reasoning, which determines the next best
...
plan in temporarily constrained environments, or plan for an multi-agent
system.

The robot memory developed in this thesis extends the capabilities of
...
\todo[inline]{ definition knowledge based system}
  
\section{RoboCup}
\label{sec:robocup}
\todo[inline]{RoboCup logo?}
...
different challenges in disaster scenarios~\cite{rc-rescue} and
...
related objects, such as screws and nuts~\cite{rc-work}. In the
...
\emph{RoboCup@Home} league featuring service robots in a domestic
...
dynamism of the environment is caused by the randomization of factory
...
example the fridge, can be very clouded.
\todo[inline]{introduce Caesar?}
...
receive all messages sent on it. Messages are encoded using the Google
Protocol Buffers (Protobuf) message protocol, which enforces a fixed message
structure. A central process called \texttt{Roscoe} manages the
registration of publishers and subscribers. It is also responsible for
...
the removing the synchronization from CLIPS decreases the
agent's complexity.
...
is a universally quantified and conditional, but not a full FOL
...
well as visualization and simulation, it is a convenient choice for a
...
are central tasks of this thesis, the chaise of a database system is
...
can directly operate on the graph. Thus costly join operations, which
...
to be stored. Furthermore, this simplifies development in which the
...
\label{sec:arangodb}
ArangoDB is an open source database managing system. What sets it

...
A robot is \textit{a machine capable of automatically carrying out a
  complex series of movements}~\cite{robot-dict} and belongs to the
group of \textit{cyber physical systems (CPS)}, which can be defined
as \textit{computing elements combined with physical sensing and
...
robots that are \textit{capable of moving in their environment} and
are \textit{capable of operating without any form of external control
...
designed for. They are \textit{agents}, which utilize a
\textit{sense-think-act cycle}. Sensors are used to percieve the
...
acts depending on its sensing depends on its \textit{agent
  architecture}. Russell and Norvig differentiate the class of simple
\textit{reflex agents} and the class of more advanced
\textit{model-based, goal-based, and learning
...
We define a \textit{robot memory} as a system that memorizes the
...
Physically, a robot memory can be stored on \textit{volatile} or
\textit{non-volatile} devices that can be read and written. A volatile
device, such as \textit{Random-access memory (RAM)}, can only store
...
\textit{multiagent system}, a system that is composed of multiple
...
\textit{distributed}. Furthermore, we will also call a system composed
of multiple interacting autonomous mobile robots a \textit{multi-robot
...
\textit{centralized} or \textit{distributed}. In the centralized one,
...
\textit{consistency}, which requires that information stored in a
...
\subsection{CLIPS Rules Engine} The reasoner currently used by
...
\subsection{Planning Domain Definition Language (PDDL)} PDDL is a
...
%% \subsection{Golog} Golog is a high-level programming language based on the

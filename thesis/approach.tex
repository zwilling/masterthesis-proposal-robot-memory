...
the KBS, the robot memory features have to be provided in different
programming languages with different types of information
representation \todo{say knowledge representation?} and
limitations. For example, the representation as unordered facts in
CLIPS is similar to the key-value structure of documents but does not
allow nesting, and the representation as predicates in PDDL requires
the mapping from documents to predicates as shown in
...
transformation can also require deriving symbolic information
(e.g. the room a robot is in) if the robot memory only contains
spatio-temporal information about the position of the robot in 3D
coordinates. Furthermore, KBS often are specialized on some area, such
as planning or world model reasoning. Therefore, only a part of the
information contained in the robot memory might be relevant to the
KBS. The interface has to filter out non relevant parts.  Due to the
difference of KBS, each KBS language requires a separate interface to
the robot memory. This separate interface can be provided by a
separate component using the robot memory and the environment of the
KBS. It can also bridge to specific concepts used by the KSB, such as
the problem definition in PDDL instead of the procedural interaction
with the robot memory. The filtering of the relevant part of the robot
memory can use additional query constraints or be limited to specific
collections. It is also posible to rename keys of key-value pairs in
documents to obtain the correct identifyers used in the KBS.
...
system. This enables sharing all neccessary information while saving
...
instance that executes adn mutually locks all write operations in the
...
infomration by having a copy on each robot, and reintegrate the droped
...
use the computable has to register it on it's own robot memory instance.

...
plotted. Both mongod processes, one for the robot local robot memory
and one for the distributed worldmodel, have a CPU usage below $1\%$
and a memory usage below $40MB$. This is not suprising because the
...
and there are few new documents for incoming orders and and workpieces
...
\todo[inline]{Comparison to Synchronization with protobuf messages
  (resyncing feature of MongoDB, not always sending whole world
  model...)}
The size of the database on the harddrive is about $1097MB$ for the
distributed part and $1060MB$ for each local one. The size does not
change noticably over time. $1GB$ of each database is occupied by the

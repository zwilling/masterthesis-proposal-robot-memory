...
\todo[inline]{check consistency after reordering}
...
paragraph. \reffig{fig:computable-durations} shows that
...
operation durations because is the worst case queries do not benefit
...
\begin{figure}[ht!]
  ...
  \end{subfigure}
  \caption[Duration of robot memory operations with indexing]{Duration of robot memory operations with indexing}
  \label{fig:eval-indexing}
\end{figure}
As \reffig{fig:eval-indexing} shows in comparison to
\reffig{fig:eval-durations-1}, indexing by the object name has a large
impact on operation durations. The scales used both figures
are identical for the same operation. The largest benefit can be
observed for queries and updates. Both are faster if the query is
restricting the value of the key used as primary index. If the query
only restricts other key-value fields, such as the query about places
in \reffig{fig:query-index}, query durations are not noticeably
...
larger with indexing because MongoDB has to sort in new documents at
the proper position. The durations for computables do not change in

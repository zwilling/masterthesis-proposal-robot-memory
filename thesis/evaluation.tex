...
allowing any computation function to be interfaceable by the robot
...
\todo[inline]{synchronization mongodb changes, instead of changes and
  full wm in protobuf}
more convenient than implementing and strongly typing a message
...
\subsection{Triggers:}
By using triggers, components can properly be notified about changes
...
computation time, triggers analyze the changes caused by single
...
important use cases, such as keeping a world model updated, and
...
an arbitrary function yields a specified result for some input
...
worked well in both application scenarios and are convenient for
...
allowed simple and efficient monitoring during the development of the
application scenarios. On the other hand, the robot memory can be
...
activation when specified in the configuration. It is possible to

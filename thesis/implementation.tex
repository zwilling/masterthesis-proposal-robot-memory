...
\texttt{\$set} sub-document.\todo[inline]{example update with \$set?} In contrast to
...
is matched by a computable spec, the robot memory query is inserted
as document in a separate and empty collection. When this inserted
document is returned by performing the computable spec as query on the

...
%\todo[inline]{example expressive query?}
...
computable manager.
The example in \reflst{lst:unit-test} shows how this computable can be
...
%\todo[inline]{example BB computable result}
...
value. The test shown in \reflst{lst:unit-test} checks if computables work by registering the
...
contains the correct sum from \reflst{lst:comp}c. Unit-testing in Fawkes is based on Google's
...
theoretical foundation. \reflst{lst:clips-map} shows how a document is

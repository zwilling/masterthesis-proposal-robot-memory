...
\label{sec:impl}
\todo[inline]{Here are probably more small mistakes, because I have not checked this yet}
...
\todo[inline]{visualization/human interface by Robomongo}
...
\begin{wrapfigure}{r}{0.42\textwidth}{6cm}
  \vspace{-0.8cm}
\begin{lstlisting}[style=SmallJSON,
...
the \texttt{clips-robot-memory} plugin. \todo[inline]{explain
  CLIPS-feature?} In contrast to PDDL, CLIPS can use procedures. Thus
...
In \resec{sec:app-rcll}, we present how the robot memory is used in
...
\hfill
  ...
  \label{fig:rcll}
\end{figure}
\reffig{fig:rcll-real} shows the factory area and the logistics robots
of the Carologistics and Solidus teams at the RoboCup finals in
...
usage is identical in simulation and real world. A central component
of the Carologistics robot software in the RCLL is the CLIPS agent. It
...
of robot actions. The decision approach and world model maintenance
is distributed. Robots have to coordinate with each other and notify
each other about changes in the environment. Furthermore, they can
drop out and might be restarted. Thus, the CLIPS agent needs to
synchronize its world model with the other robots. The existing
implementation used by Carologistics in 2016 uses Protobuf messages
which are being sent between the agents. In this application scenario,
we exchange the synchronization via Protobuf by using the robot
memory. Thus the world model of CLIPS has to be represented in the
robot memory and distributed over all robots. On the one hand, this
lays the foundation for using the robot memory in the RCLL in future
work. On the other hand, this simplifies the CLIPS agent by separating
the synchronization. The synchronization by
MongoDB is advantageous because it only sends necessary changes over the
network instead of the whole world model, as it is done by the CLIPS
agent. Another advantage is the persistency, because robots keep their
state if one or all of them are restarted.
...
description task
classical planning problem + difficulties with practical
  implementation (occlusion)
image real world
explain @home relation
components collaborating over robot memory
perception plugin , bbinterface xteon, block positions
PDDL domain description actions, problem description generated from
  robot memory
Computables for on-table, clear, unknown blocks
Transformation geometric to symbolic 
CLIPS executive, starts planner, trigger for new plan
executes plan, task-step
motion planner for arm motion
image openrave
scene construction, transform computable
updates world model -> remembers on and position of covered blocks
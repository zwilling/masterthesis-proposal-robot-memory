...
as a string concatinated by clips and yields a cursor to documents
...
accoring to changes in the robot memory. However, executing callback
...
base. This fact contains an identifyer defined during the registration
...
internal representation of the robot's evironment, the
...
memory. Based on config values, it queries the robot memory for
...
usage is identical in simulaiton and real world. A central component
...
of robot actions. The decision approach and world model maintainance
...
work. On the other hand, this simplyfies the CLIPS agent by separating
the synchronization. The synchronization by
MongoDB is advantageous because it only sends neccessary changes over the
...
initialized at the start of the game and has to be transfered to the
...
triggers. Each agent registeres a trigger at its own
